...
namespace DDOOCP_Assignment
{
    public partial class Cycling : Form
    {
        ...
        private string name = "Bicycling";
        public Cycling(User user, Target target)
        {
            InitializeComponent();
            this.user = user;
            this.target = target;
        ...
        private void cal_btn_Click(object sender, EventArgs e)
        {
            ...
            double timeBYhours = time / 60;

            double speed = distance/timeBYhours;

            DateTime currentDateTime = DateTime.Today;
            string formattedDate = currentDateTime.ToString("yyyy-MM-dd");

            double caloriesBurned = (0.35 * target.Current_weight * MET) * distance/speed;
